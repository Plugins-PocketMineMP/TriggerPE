...
class AreaTriggerCommand extends Command{
	...
	public function __construct(){
		...
		$this->setPermission("triggerpe.command.vta");
	}
					...
					if(trim($args[1] ?? "") !== ""){
						if(!(TriggerPE::getInstance()->getTriggerFactory()->getTrigger($args[1])) instanceof Trigger){
							if(in_array($args[2] ?? "", Trigger::ACTION_LIST)){
								if(trim($args[3] ?? "") !== ""){
									TriggerQueue::addCreateQueue($sender, $args[1], $args[3], Trigger::TYPE_AREA, $args[2]);
						...
						TriggerPE::message($sender, "Usage: /vta add <name> <actionType> <action>");
					}
					break;
				case "remove":
					if(trim($args[1] ?? "") !== ""){
						if(($trigger = TriggerPE::getInstance()->getTriggerFactory()->getTrigger($args[1])) instanceof Trigger){
							if($trigger instanceof AreaTrigger){
								...
								TriggerPE::message($sender, "That trigger is not a area trigger.");
							}
						}else{
							TriggerPE::message($sender, "No trigger with that name exists.");
						}
					}else{
						TriggerPE::message($sender, "Usage: /vta remove <name>");
					}
					break;
				case "list":
					$list = [];
					...
					break;
				default:
					TriggerPE::message($sender, "/vta add <name> <actionType> <action>");
					TriggerPE::message($sender, "/vta remove <name>");
					TriggerPE::message($sender, "/vta list");
			}
		}
		return true;
	}
